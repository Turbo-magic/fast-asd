...
def create_video_segments(file_path, scene_future, start_time=0, end_time=None, fps=30, original_video_length=None):
    ...
    for scene in scene_outputs:
        ...
        video_segments.append(VideoSegment(path=str(file_path), start=start_seconds, end=end_seconds, start_frame=start_frame, end_frame=end_frame))

    if len(video_segments) == 0:
        if end_time is None:
            if original_video_length is None:
                original_video_length = get_video_length(str(file_path))
            end_time = original_video_length
        video_segments.append(VideoSegment(path=str(file_path), start=start_time, end=end_time))
